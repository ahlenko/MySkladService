...
public class A_C_Packing extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.f2_packing);
        Vibrator vibrator = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);
            ...
            protected Void doInBackground(Void... params) {
                try {
                    ...
                    DialogsViewer.twoButtonDialog(
                            context, new Intent(A_C_Packing.this, A_C_Packing.class),
        ...
        btn_print.setOnClickListener(enter->{
            ...
            String massage = "Друк ТТН для замовлення: " + text_info.getText().toString() + "\n Привласнено код: " + TTNCode.get();
                ...
                public void run() {
                    try {
                        ...
                        runOnUiThread(new Runnable() {
                            public void run() {
                                DialogsViewer.twoButtonDialog(
                                        context, new Intent(A_C_Packing.this, A_C_Packing.class), activity, "Помилка",
        ...
        class CheckingPrint extends AsyncTask<Void, Void, Void> {
            ...
            protected Void doInBackground(Void... params) {
                try {
                    ...
                    resultSet = MS_SQLSelect.ReadOrderCode(mssqlConnection, checker.GetChecker());
                    resultSet.next(); if(resultSet.getInt("state") != 0 ) checker.ChangePrivace(true);
                    OrderCode = resultSet.getInt("code");
                    resultSet = MS_SQLSelect.ReadOrderElements(mssqlConnection, checker.GetChecker());

                    int i = 1; while (resultSet.next()) {
                        View temp = getLayoutInflater().inflate(R.layout.template_cur_packing, TableView, false);
        ...
        CheckingPrint checkingPrint = new CheckingPrint(new TaskInterface() {
            ...
            public void onTaskComplete() {
                ...
                if (View_s.isEmpty()) {
                    infostate.setText("Список товарів замовлення порожній");
                } else {
