...
public class A_T_Packing extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.d2_packing_table);
        Vibrator vibrator = (Vibrator) getSystemService(Context.VIBRATOR_SERVICE);
            ...
            protected Void doInBackground(Void... params) {
                try {
                    ...
                    DialogsViewer.twoButtonDialog(
                            context, new Intent(A_T_Packing.this, A_T_Packing.class),
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            String dateS = currentDate.getDayOfMonth() + " " + month[currentDate.getMonthValue()-1] + " " + currentDate.getYear();
        ...
        class CheckingPrint extends AsyncTask<Void, Void, Void> {
            ...
            protected Void doInBackground(Void... params) {
                try {
                    ...
                    resultSet.next(); int company = resultSet.getInt("id");
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                        resultSet = MS_SQLSelect.ReadOrders(mssqlConnection, company, newDate.get());
                    }

                    int iter = 0;
                    while (resultSet.next()) {
                        View temp = getLayoutInflater().inflate(R.layout.template_view_packing, TableView, false);
                        ...
                        switch (resultSet.getInt("state")){
                            case 0:
                                state.setText("Не оброблено");
                                printBtn.setAlpha(0.4f);
                                break;
                            case 1:
                                state.setText("Обробляється");
                                packBtn.setAlpha(0.4f);
                                printBtn.setAlpha(0.4f);
                                break;
                            case 2:
                                state.setText("Оброблено");
                                temp.setAlpha(0.4f);
                                break;
                        }
        ...
        CheckingPrint checkingPrint = new CheckingPrint(new TaskInterface() {
            ...
            public void onTaskComplete() {
                ...
                if (View_s.isEmpty()) {
                    infostate.setText("Дані про замовлення відсутні");
                } else{
        ...
        button_next.setOnClickListener (enter -> {
            ...
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
                String dateS = newDate.get().getDayOfMonth() + " " + month[newDate.get().getMonthValue()-1] + " " + newDate.get().getYear();
                text_info.setText(dateS);
            } vibrator.vibrate(50);
